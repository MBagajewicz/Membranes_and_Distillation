...
def fun_initial_Aspen(file_name, stream_name1, stream_name2, block_name, comp_name, z_feed1, F_feed1, T_feed1,z_feed2, F_feed2, T_feed2, P_col, x_TOP, x_BOTTOM,
    ...
    # Set feed1 stream data:
    for comp, mole_frac in zip(comp_name, z_feed1):  # Molar fractions for components
        molar_flow = F_feed1 * mole_frac  # Calculate molar flow for the component
        Aspen.Tree.FindNode(
            rf'\Data\Streams\{stream_name1}\Input\FLOW\MIXED\{comp}').Value = molar_flow  # Input molar flow for each component
    Aspen.Tree.FindNode(
        rf'\Data\Streams\{stream_name1}\Input\TEMP\MIXED').Value = T_feed1  # Input feed stream temperature (K)
    Aspen.Tree.FindNode(
        rf'\Data\Streams\{stream_name1}\Input\PRES\MIXED').Value = P_col  # Input feed stream pressure (Pa)

   # Set feed2 stream data:
    for comp, mole_frac in zip(comp_name, z_feed2):  # Molar fractions for components
        molar_flow = F_feed2 * mole_frac  # Calculate molar flow for the component
        Aspen.Tree.FindNode(
            rf'\Data\Streams\{stream_name2}\Input\FLOW\MIXED\{comp}').Value = molar_flow  # Input molar flow for each component
    Aspen.Tree.FindNode(
        rf'\Data\Streams\{stream_name2}\Input\TEMP\MIXED').Value = T_feed2  # Input feed stream temperature (K)
    Aspen.Tree.FindNode(
        rf'\Data\Streams\{stream_name2}\Input\PRES\MIXED').Value = P_col  # Input feed stream pressure (Pa)
    ...
    Aspen.Tree.FindNode(
        rf'\Data\Blocks\{block_name}\Input\VALUE\1').Value = x_TOP  # Top product specification (mole frac)
    Aspen.Tree.FindNode(
        rf'\Data\Blocks\{block_name}\Input\VALUE\2').Value = x_BOTTOM  # Bottom product specification (mole frac)
...
def fun_run_Aspen(v_Ns, v_Nf1, v_Nf2, Aspen, block_name, stream_names, components, v_Nc):
    ...
    if is_error == 0:
        print(f'Simulation converged successfully for Ns = {v_Ns}, Nf1 = {v_Nf1}, Nf2 = {v_Nf2}')
        ...
        if message == 'completed with warnings:':
            ...
            print(f'Aspen Simulation for candidate Ns = {v_Ns}, Nf1 = {v_Nf1}, Nf2 = {v_Nf2} was completed with warnings')
            ...
            print(f'Simulation did NOT converged for Ns = {v_Ns}, Nf1 = {v_Nf1}, Nf2 = {v_Nf2}', 'Reseting and trying again')
        ...
        if is_error == 0:
            ...
            print(
                f'Simulation did NOT converged for Ns = {v_Ns}, Nf1 = {v_Nf1}, Nf2 = {v_Nf2} on the second try, reseting for next run')
            Aspen.Reinit()
...
def fun_getfromAspen(Aspen, block_name, distillate_name, v_Ns, components, v_Nc):
    ...
    for st in range(1, v_Ns + 1):
        ...
        vapor_mole_flows[st - 1] = Aspen.Tree.FindNode(
            rf'\Data\Blocks\{block_name}\Output\VAP_FLOW\{st}').Value  # kmol/h
        liquid_mole_flows[st - 1] = Aspen.Tree.FindNode(
            rf'\Data\Blocks\{block_name}\Output\LIQ_FLOW\{st}').Value  # kmol/h
        vapor_mass_flows[st - 1] = Aspen.Tree.FindNode(
            rf'\Data\Blocks\{block_name}\Output\VAP_FLOW_MS\{st}').Value  # kg/h
        liquid_mass_flows[st - 1] = Aspen.Tree.FindNode(
            rf'\Data\Blocks\{block_name}\Output\LIQ_FLOW_MS\{st}').Value  # kg/h
    ...
    condenser_duty = abs(
        Aspen.Tree.FindNode(rf'\Data\Blocks\{block_name}\Output\COND_DUTY').Value) * 1000 / 3600  # kJ/h to W
    reboiler_duty = Aspen.Tree.FindNode(rf'\Data\Blocks\{block_name}\Output\REB_DUTY').Value * 1000 / 3600  # kJ/h to W
    distillate_liquid_molar_density = Aspen.Tree.FindNode(
        rf'\Data\Streams\{distillate_name}\Output\RHO_LIQ').Value * 1000  # distillate liquid molar density in mol/cm³ converted to kmol/m³
    distillate_liquid_molar_weight = Aspen.Tree.FindNode(
        rf'\Data\Streams\{distillate_name}\Output\MW_LIQ').Value  # distillateliquid molar weight
    ...
    liquid_molar_density = Aspen.Tree.FindNode(
        rf'\Data\Blocks\{block_name}\Output\RHO_LIQ\{ArgV}').Value * 1000  # liquid molar density in mol/cm³ converted to kmol/m³
    liquid_molar_weight = Aspen.Tree.FindNode(
        rf"\Data\Blocks\{block_name}\Output\MW_LIQ\{ArgV}").Value  # liquid molar weight
    vapor_molar_density = Aspen.Tree.FindNode(
        rf'\Data\Blocks\{block_name}\Output\RHO_GAS\{ArgV}').Value * 1000  # vapor molar density in mol/cm³ converted to kmol/m³
    vapor_molar_weight = Aspen.Tree.FindNode(
        rf"\Data\Blocks\{block_name}\Output\MW_GAS\{ArgV}").Value  # vapor molar weight
...
def fun_getfromAspen_hydraulics(v_Ns, Aspen, block_name):
    ...
    for st in range(2, v_Ns):
        density_liquid[st - 2] = Aspen.Tree.FindNode(
            rf'\Data\Blocks\{block_name}\Output\HYD_RHOL\{st}').Value * 1000  # g/cm³ to kg/m³
        density_vapor[st - 2] = Aspen.Tree.FindNode(
            rf'\Data\Blocks\{block_name}\Output\HYD_RHOV\{st}').Value * 1000  # g/cm³ to kg/m³
        surface_tension[st - 2] = Aspen.Tree.FindNode(
            rf'\Data\Blocks\{block_name}\Output\HYD_STEN\{st}').Value / 1000  # dyne/cm to N/m
...
def fun_getfromAspen_condenser(Aspen, block_name, stream_name):
    ...
    Condenser_hot_stream['liquid_density'] = Aspen.Tree.FindNode(
        rf'\Data\Blocks\{block_name}\Output\RHO_LIQ\1').Value * 1000 * MW  # mol/cm³ to kmol/m³, then to kg/m³
    Condenser_hot_stream['vapor_density'] = Aspen.Tree.FindNode(
        rf'\Data\Blocks\{block_name}\Output\RHO_GAS\2').Value * 1000 * MW  # mol/cm³ to kmol/m³, then to kg/m³
    Condenser_hot_stream['liquid_viscosity'] = Aspen.Tree.FindNode(
        rf'\Data\Blocks\{block_name}\Output\HYD_MUL\1').Value / 1000  # cP to Pa.s
    Condenser_hot_stream['vapor_viscosity'] = Aspen.Tree.FindNode(
        rf'\Data\Blocks\{block_name}\Output\HYD_MUV\2').Value / 1000  # cP to Pa.s
    Condenser_hot_stream['thermal_conductivity'] = Aspen.Tree.FindNode(
        rf"\Data\Streams\{stream_name}\Output\STRM_UPP\KMX\MIXED\LIQUID").Value  # W/(m.K)
...
def fun_getfromAspen_reboiler(Aspen, stream_name):
    ...
    Reboiler_cold_stream['critical_pressure'] = Aspen.Tree.FindNode(
        rf"\Data\Streams\{stream_name}\Output\STRM_UPP\PCMX\MIXED\TOTAL").Value  # Pa
